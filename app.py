...
# ─── Константы и Иконки (Добавлены все банки) ─────────────────────────────────
CURRENCY_ICONS = {
...
BANK_ICONS = {
    ...
    "Демир Банк": "🔵", "MBank": "🟣",
    "Керемет Банк": "🟡", "НБКР": "🏛️",
    "Кыргызалтын": "🥇", "Бакай Банк": "🦅",
    "Бакай": "🦅", "Азия Банк": "🅰️",
    "Халык Банк": "🟩", "КСБ Банк": "🛡️",
    "Элдик Банк": "🟦", "Айыл Банк": "🌾",
    "Компаньон": "🍎", "ФИНКА Банк": "🦉"
}
...
def fmt_bank(b: str) -> str:
    icon = BANK_ICONS.get(b, "🏦") # Если банка нет в списке иконок, поставит стандартный эмодзи
    return f"{icon} {b}"

@st.cache_data(ttl=600)
def load_current_data(table_name: str) -> pd.DataFrame:
    """
    Умная загрузка: ищет самую свежую дату во временной таблице.
    Если там пусто (после ночной очистки), берет последние данные из истории.
    """
    # 1. Проверяем самую свежую дату во временной таблице
    res_date = sb.table(table_name).select("date").order("date", desc=True).limit(1).execute()
    
    # 2. Если временная таблица пуста, обращаемся к истории
    if not res_date.data:
        hist_table = "historical_" + table_name
        res_hist = sb.table(hist_table).select("date").order("date", desc=True).limit(1).execute()
        if not res_hist.data:
            return pd.DataFrame() # Если и в истории пусто
        latest_date = res_hist.data[0]["date"]
        res = sb.table(hist_table).select("*").eq("date", latest_date).execute()
    else:
        # 3. Если данные есть, грузим их
...
st.markdown("Сравнение рыночных курсов со всеми доступными банками КР.")
...
with tab1:
    if df_rates.empty:
        st.warning("⚠️ Данных по курсам валют пока нет. Ожидается запуск парсера.")
    else:
        # Фильтры
        ...
        if df_filtered.empty:
            ...
            with c1:
                ...
                if not dcb_data.empty:
                    ...
                    st.write("Нет данных за сегодня")
            ...
            with c3:
                ...
                st.metric("Самая дорогая покупка", f"{best_buy:.2f}")
                st.metric("Самая дешевая продажа", f"{best_sell:.2f}")

            st.divider()
            
            st.markdown("#### Все банки")
            # Выводим абсолютно все банки, которые есть в датафрейме
...
with tab2:
    if df_gold.empty:
        st.warning("⚠️ Данных по золотым слиткам пока нет. Ожидается запуск парсера.")
    else:
...
with tab3:
    ...
    if h_mode == "Курсы валют":
        ...
        if hist.empty:
            ...
            # Выделяем все банки по умолчанию, чтобы график сразу был красивым
            all_hist_banks = sorted(hist["bank_name"].unique().tolist())
            bank_filter = st.multiselect(
                "Банки для графика",
                all_hist_banks,
                default=all_hist_banks,
            )
        ...
        if hist_gold.empty:
            ...
            all_gold_banks = sorted(hist_gold["bank_name"].unique().tolist())
            g_bank_filter = st.multiselect(
                "Учреждения для графика",
                all_gold_banks,
                default=all_gold_banks,
            )
...
st.divider()
st.caption(f"Данные собираются автоматически. Текущая дата сервера: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")